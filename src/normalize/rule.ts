 ...
 */
export default function normalizeRule(editor: Editor, entry: NodeEntry): boolean {
    ...
    if (Rule.isRule(node)) {
        ...
        if (normalizeOrderedChildren(
            editor,
            [node, path],
            [Title.isTitle, Statement.isStatement, Proof.isProof, RuleExample.isRuleExample],
...
function normalizeInvalidChild(editor: Editor, entry: NodeEntry, parent: Rule): void {
    const [, path] = entry
