 ...
 */
export default function normalizeExercise(editor: Editor, entry: NodeEntry): boolean {
    ...
    if (Exercise.isExercise(node)) {
        ...
        if (normalizeOrderedChildren(
            editor,
            [node, path],
            [Problem.isProblem, Solution.isSolution, Commentary.isCommentary],
...
function normalizeInvalidChild(editor: Editor, entry: NodeEntry, parent: Exercise): void {
    const [, path] = entry
