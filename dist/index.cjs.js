...
function onBackspace(editor, ev) {
    ...
    const [admonition, admonitionPath] = (_a = slate.Editor.above(editor, { match: Admonition.isAdmonition })) !== null && _a !== void 0 ? _a : [];
    if (admonition != null && admonitionPath != null) {
        ...
        = (_b = slate.Editor.above(editor, { match: slateLists.ListItem.isListItem })) !== null && _b !== void 0 ? _b : [];
        if (listPath != null) {
            const pointBefore = slate.Editor.before(editor, selection, { unit: 'character' });
            if (pointBefore && slate.Editor.above(editor, { at: pointBefore.path, match: slateLists.ListItem.isListItem }))
                return;
    ...
    const [code, codePath] = (_c = slate.Editor.above(editor, { match: isCodeLike })) !== null && _c !== void 0 ? _c : [];
    if (code != null) {
    ...
    const [alt] = (_d = slate.Editor.above(editor, { match: AltText.isAltText })) !== null && _d !== void 0 ? _d : [];
    if (alt != null) {
        if (selection.anchor.offset > 0)
            return;
...
function onEnter(editor, ev) {
    ...
    if (isCodeLike(block)) {
        slate.Editor.withoutNormalizing(editor, () => {
            ...
            if (!ev.shiftKey) {
                if (slate.Node.string(slate.Editor.node(editor, selection)[0]).endsWith('\n\n')) {
                    slate.Transforms.delete(editor, { distance: 2, unit: 'character', reverse: true });
                    slate.Transforms.insertNodes(editor, {
                        type: 'paragraph',
                        children: [
                            { text: '' },
                    ...
                    slate.Transforms.removeNodes(editor, { at: blockPath });
                    slate.Transforms.move(editor, { distance: 1, unit: 'character' });
                }
            }
        });
        return ev.preventDefault();
...
function normalizeGlossary(editor, entry) {
    ...
    if (Definition.isDefinition(node)) {
        ...
        if (DefinitionTerm.isDefinitionTerm(node.children[1])) {
            slate.Transforms.mergeNodes(editor, { at: [...path, 1] });
            return true;
        }
    }
...
function wrapSectionTitle(editor, options = {}) {
    slate.Editor.withoutNormalizing(editor, () => {
        ...
        if (at == null)
            return;
        const range = slate.Editor.range(editor, at);
        // Find location for the new section
        /* eslint-disable-next-line
            @typescript-eslint/no-extra-non-null-assertion --
            Editor is above every path, so this cannot return null */
        const [parent, parentPath] = slate.Editor.above(editor, {
            at: range,
            match: node => Section.isSection(node) || slate.Editor.isEditor(node),
        });
        // Find the node to transform into new section's title
        const [maybeTitle, maybeTitlePath] = (_a = slate.Editor.above(editor, {
            at: range,
        ...
        // Insert new section
        slate.Transforms.insertNodes(editor, { type: 'section', children: [] }, { at: [...parentPath, sectionIndex] });
        const sectionPath = slate.Editor.pathRef(editor, [...parentPath, sectionIndex]);
        ...
        if (slate.Range.isCollapsed(range)) {
            ...
            if (!slate.Editor.isEnd(editor, end, maybeTitlePath)) {
                slate.Transforms.splitNodes(editor, {
                    at: end,
                    match: node => node === container,
                });
                container = slate.Node.ancestor(editor, maybeTitlePath);
            }
            if (!slate.Editor.isStart(editor, start, maybeTitlePath)) {
                titlePath = slate.Path.next(maybeTitlePath);
                slate.Transforms.splitNodes(editor, {
                    at: start,
                    match: node => node === container,
                });
            ...
            slate.Transforms.select(editor, titlePath);
        }
        slate.Transforms.setNodes(editor, { type: 'title' }, { at: titlePath });
