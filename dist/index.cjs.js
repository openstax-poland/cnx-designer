...
function onEnter(editor, ev) {
    ...
    const [altText, altTextPath] = (_d = slate.Editor.above(editor, { match: AltText.isAltText })) !== null && _d !== void 0 ? _d : [];
    if (altText != null && altTextPath != null && slate.Range.isCollapsed(selection)) {
        const after = slate.Editor.after(editor, altTextPath);
        if (!after)
            return;
        const newSelection = {
            anchor: {
    ...
    const [listItem, listItemPath] = (_e = slate.Editor.above(editor, { match: slateLists.ListItem.isListItem })) !== null && _e !== void 0 ? _e : [];
    if (listItem != null && listItemPath != null && slate.Range.isCollapsed(selection)) {
        if (slate.Node.string(listItem) !== '')
            return;
        const after = slate.Editor.after(editor, listItemPath);
    ...
    if (slate.Range.isCollapsed(selection)) {
        ...
        const [container, containerPath] = (_f = slate.Editor.above(editor, {
