...
import { Admonition, AltText, Audio, Caption, Figure, Image, ListItem, Media, Problem, Section, Solution, Title, Video, } from '../interfaces';
 ...
 */
export default function normalizeFigure(editor, entry) {
    ...
    if (Figure.isFigure(node)) {
        ...
        if (normalizeOrderedChildren(editor, [node, path], [Title.isTitle, isFigureItem, Caption.isCaption], normalizeInvalidChild)) {
            return true;
        }
    ...
    if (Media.isMedia(node)) {
        ...
        if (normalizeOrderedChildren(editor, [node, path], [isMediaItem, AltText.isAltText], normalizeInvalidChild)) {
            return true;
        }
...
function normalizeInvalidChild(editor, entry) {
    const [node, path] = entry;
    if (Text.isText(node)) {
