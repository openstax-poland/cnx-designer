import { Element, Editor, Range, Transforms, Path, Node, Text } from 'slate';
...
function onBackspace(editor, ev) {
    ...
    const [alt] = (_b = Editor.above(editor, { match: AltText.isAltText })) !== null && _b !== void 0 ? _b : [];
    if (alt != null) {
        if (selection.anchor.offset > 0)
            return;
        ...
        if (Problem.isProblem(container) || Solution.isSolution(container)
            ...
            Editor.withoutNormalizing(editor, () => {
                ...
                if (Problem.isProblem(container)) {
                    Transforms.setNodes(editor, { type: 'exercise_solution' }, { at: Path.next(containerPath) });
                }
            });
            return ev.preventDefault();
        }
        if (Meaning.isMeaning(container) || DefinitionExample.isDefinitionExample(container)) {
            Editor.withoutNormalizing(editor, () => {
                ...
                if (DefinitionExample.isDefinitionExample(container)) {
                    Transforms.setNodes(editor, { type: 'definition_meaning' }, { at: Path.next(containerPath) });
                }
            });
            return ev.preventDefault();
        }
...
export { ADMONITION_KINDS, Admonition, AltText, Audio, BulletedList, CASES, Caption, Code, Commentary, CrossReference, Definition, DefinitionExample, DefinitionTerm, DocumentReference, EnumeratedList, Equation, Exercise, Figure, Footnote, Foreign, Glossary, Image, Link, List, Meaning, Media, MediaData, MediaUse, Paragraph, Preformat, Problem, ProcessingInstruction, Proof, Quotation, RULE_KINDS, Rule, RuleExample, Section, SeeAlso, Solution, Statement, Term, Title, index as Transforms, Video, WithClasses, dedup, enumerate, isInline, isVoid, onKeyDown, unpackBlocks, uuid, withCnx, withIds };
