import { Element, Range, Editor, Transforms, Path, Text, Node } from 'slate';
import { List as List$1 } from 'slate-lists';
...
function onEnter(editor, ev) {
    ...
    const [caption, captionPath] = (_b = Editor.above(editor, { match: Caption.isCaption })) !== null && _b !== void 0 ? _b : [];
    if (caption != null) {
        Editor.withoutNormalizing(editor, () => {
            if (!Range.isCollapsed(selection)) {
                Editor.deleteFragment(editor);
    ...
    if (Range.isCollapsed(selection)) {
        ...
        const [container, containerPath] = (_c = Editor.above(editor, {
...
function normalizeFigure(editor, entry) {
    ...
    if (Figure.isFigure(node)) {
        ...
        if (normalizeOrderedChildren(editor, [node, path], [isFigureItem, Caption.isCaption], normalizeInvalidChild$2)) {
            return true;
        }
    ...
    if (SeeAlso.isSeeAlso(node)) {
        ...
        for (const [child, childPath] of Node.children(editor, path)) {
            if (!Term.isTerm(child)) {
                Transforms.setNodes(editor, { type: 'definition_term' }, { at: childPath });
                return true;
            }
        }
    }
...
function normalizeStructure(editor, entry) {
    ...
    if (Section.isSection(node)) {
        ...
        if (!Title.isTitle(node.children[0])) {
            const [prev, prevPath] = (_a = Editor.previous(editor, { at: path })) !== null && _a !== void 0 ? _a : [];
...
export { ADMONITION_KINDS, Admonition, AltText, Audio, BulletedList, CASES, Caption, Code, Commentary, CrossReference, Definition, DefinitionExample, DefinitionTerm, DocumentReference, EnumeratedList, Exercise, Figure, Footnote, Foreign, Glossary, Image, Link, List, Meaning, Media, MediaData, MediaUse, Paragraph, Preformat, Problem, ProcessingInstruction, Proof, Quotation, RULE_KINDS, Rule, RuleExample, Section, SeeAlso, Solution, Statement, Term, Title, index as Transforms, Video, WithClasses, dedup, enumerate, isInline, isVoid, onKeyDown, uuid, withCnx, withIds };
